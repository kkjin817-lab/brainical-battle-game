...
UI는 점수·베팅 금액을 직접 변경하거나 자체 지급하지 않습니다. judge는 원금을 포함한 총지급 배율을 반환합니다. 공통 Game.mini()가 stake × multiplier를 BigInt로 계산하고 한 번만 지급합니다. 정수 배율과 분자·분모 형식의 배율을 지원합니다. 위험한 금고의 1.5배는 3/2로 저장하고 BigInt로 정확히 계산합니다.

dispose()는 모든 RAF, 타이머, 키보드 리스너를 해제해야 합니다. 정지 기록은 첫 유효 입력에서 고정하고 추가 입력을 무시합니다. 시간 제한 게임은 앱 중단 시 호스트가 `{reason:'interrupted', elapsedMs:null}`로 정산합니다. 위험한 금고는 중단 시 정산하지 않고 저장된 진행 상태를 복원합니다.

## 추첨과 복원

가중치 1인 게임 2개는 각각 50%, 가중치 1과 3은 각각 25%와 75%입니다. 현재 타이머, 주사위 굴리기, 순간 기억, 반대로 터치, 위험한 금고의 weight가 모두 1이므로 각각 20%로 등장합니다. 종류와 게임별 목표는 수식 클리어 시 한 번만 추첨하고 같은 트랜잭션에 저장합니다. 결과 화면 이동과 새로고침에서는 다시 추첨하지 않습니다.
