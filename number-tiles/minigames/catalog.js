...
(function(root){
  ...
  R.registry.register({type:'dice_roll',title:'주사위 굴리기',weight:1,
    ...
    judge:D.judge,
    offer(op){return {target:'세 칸, 세 개의 주사위',instruction:'주사위를 한 칸씩 굴려 중앙 목표 숫자와 윗면을 맞추세요. 실제 문제는 시작 후 공개돼요.',rewards:op.config.timeThresholdsMs.map((t,i)=>`${t/1000}초 이하 · ${op.config.multipliers[i]}배`)};},
  ...
  R.registry.register({
    ...
    offer(op){const c=op.config;return {target:`${(op.payload.targetMs/1000).toFixed(3)}초`,instruction:'목표 시간에 최대한 가깝게 멈추세요',rewards:[`±${(c.exactMaxMs/1000).toFixed(3)}초 · ${c.multipliers.exact}배`,`±${(c.doubleMaxMs/1000).toFixed(3)}초 · ${c.multipliers.double}배`,`±${(c.refundMaxMs/1000).toFixed(3)}초 · 원금 반환`]};},
